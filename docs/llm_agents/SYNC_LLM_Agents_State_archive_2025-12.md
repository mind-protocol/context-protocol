...
<!-- @ngram:todo IMPL→DOCS: If a shared adapter helper is introduced, expand the doc chain. -->
...
<!-- @ngram:todo Expand tool schema support and document additional provider adapters. -->
...
### Suggestions
<!-- @ngram:todo Add automated tests for tool outputs (tool_code/tool_result JSON). -->
