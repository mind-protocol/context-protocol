...
IMPL:            engine/membrane/health_check.py
```
...
flows_analysis:
  - flow_id: modulation_frame_compute
    ...
    triggers:
      - type: schedule
        source: engine/membrane/provider.py
...
```yaml
status:
  stream_destination: file:...ngram/state/SYNC_Project_Health.md
  result:
    representation: enum
    value: UNKNOWN
...
<!-- @ngram:todo Add a checker implementation in `engine/membrane/health_check.py`. -->
<!-- @ngram:todo Wire a sampled log for applied frames. -->

---

## MARKERS

<!-- @ngram:todo Decide where health results should be surfaced in CLI. -->
