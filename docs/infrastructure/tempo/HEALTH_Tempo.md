...
IMPL:            engine/infrastructure/tempo/health_check.py (planned)
```
...
flows_analysis:
  - flow_id: tempo_tick
    ...
    triggers:
      - type: schedule
        source: engine/infrastructure/tempo/tempo_controller.py:run
...
```yaml
status:
  stream_destination: file:...ngram/state/health_tempo.json
  result:
    representation: enum
    value: UNKNOWN
...
docks:
  input:
    ...
    location: engine/infrastructure/tempo/tempo_controller.py:~70
  output:
    id: canon_broadcast
    method: CanonHolder.record_to_canon
    location: engine/infrastructure/canon/canon_holder.py:~50
```
...
forwarding:
  targets:
    - location: ...ngram/state/health_tempo.json
      transport: file
      notes: consumed by ngram doctor

display:
  locations:
    - surface: CLI
      location: ngram doctor
      signal: enum
...
```yaml
manual_run:
  command: ngram health tempo --checker tempo_tick_advances
...
ngram health tempo --checker tempo_tick_advances
```
...
<!-- @ngram:todo Implement checker runner in engine/infrastructure/tempo/health_check.py -->
<!-- @ngram:todo Define real cadence thresholds per speed mode -->

---

## MARKERS

<!-- @ngram:todo Add a second indicator for canon surfacing bounds -->
<!-- @ngram:proposition capture backpressure metrics for operator visibility -->
