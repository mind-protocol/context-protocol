...
What `add-framework init` does:
1. Copies the protocol files to .add-framework/ in your project
2. Appends the protocol bootstrap to your CLAUDE.md (creates it if missing)

What `add-framework validate` does:
...
- Versioning: `pip install --upgrade add-framework` gets you updates
- Discoverability: easier to remember than a git URL
- Future: can add `validate`, `new-module`, etc. without changing workflow

TEMPLATES LOCATION:
- When installed: src/add_framework/templates/ (bundled with package)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="add-framework",
        description="ADD Framework - A context management system for AI agents"
    )
    ...
    init_parser = subparsers.add_parser(
        "init",
        help="Initialize ADD Framework in the current directory"
    )
    init_parser.add_argument(
        "--force", "-f",
        action="store_true",
        help="Overwrite existing .add-framework/ directory"
    )
    init_parser.add_argument(
        "--dir", "-d",
        type=Path,
    ...
    if args.command == "init":
        ...
        if args.list:
            ...
            if not ignores:
                ...
                print(f"Add ignores with: add-framework ignore --type TYPE --path PATH --reason REASON")
            else:
            ...
            print("Usage:")
            print("  List ignores: add-framework ignore --list")
            print("  Add ignore:   add-framework ignore --type TYPE --path PATH [--reason REASON]")
            print()
            print("Examples:")
            print("  add-framework ignore --type MONOLITH --path src/legacy.py --reason 'Legacy code, too risky to split'")
            print("  add-framework ignore --type MAGIC_VALUES --path tests/** --reason 'Test fixtures'")
            sys.exit(1)
