"""
Init command for ADD Framework CLI.

Initializes the ADD Framework in a project directory by:
- Copying protocol files to .add-framework/
...
def init_protocol(target_dir: Path, force: bool = False, claude_md_dir: Path = None) -> bool:
    """
    Initialize the ADD Framework in a project directory.

    Copies protocol files and updates CLAUDE.md.

    Args:
        target_dir: The project directory to initialize
        force: If True, overwrite existing .add-framework/
    ...
    # Source paths
    protocol_source = templates_path / "add-framework"
    claude_addition = templates_path / "CLAUDE_ADDITION.md"
    modules_yaml_source = templates_path / "modules.yaml"
    ignore_source = templates_path / "add-frameworkignore"

    # Destination paths
    protocol_dest = target_dir / ".add-framework"
    modules_yaml_dest = target_dir / "modules.yaml"
    ignore_dest = target_dir / ".add-frameworkignore"
    ...
    # Copy .add-frameworkignore to project root (if not exists or force)
...
"""
    ...
    if claude_md.exists():
        ...
        if "@.add-framework/PRINCIPLES.md" in existing_content or "## ADD Framework" in existing_content:
            print(f"○ {claude_md} already has ADD Framework section")
        else:
    ...
    print()
    print("ADD Framework initialized!")
    print()
    print("Next steps:")
    print("  1. Read .add-framework/PROTOCOL.md")
    print("  2. Update .add-framework/state/SYNC_Project_State.md")
    print("  3. Load the VIEW matching your task when working")
    print()
    print("To bootstrap an LLM, run:")
    print(f"  add-framework prompt --dir {target_dir}")
