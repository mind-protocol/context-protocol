...
def get_issue_explanation(issue_type: str) -> Dict[str, str]:
    ...
    explanations = {
        ...
        "INCOMPLETE_CHAIN": {
            ...
            "action": "Create the missing doc types using templates from .add-framework/templates/.",
        },
        "NO_DOCS_REF": {
            "risk": "Without a DOCS: reference, the bidirectional link is broken. Agents reading code can't find the design docs, and `add-framework context` won't work.",
...
def generate_health_markdown(results: Dict[str, Any], github_issues: List = None) -> str:
    ...
    lines.append("UPDATED_BY: add-framework doctor")
    score = results['score']
    if score >= 80:
        status = "HEALTHY"
    elif score >= 50:
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by `add-framework doctor`*")
...
def check_sync_status(target_dir: Path) -> Dict[str, int]:
    ...
    search_paths = [
        target_dir / ".add-framework" / "state",
        target_dir / "docs",
    ]
