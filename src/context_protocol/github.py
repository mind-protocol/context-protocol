"""
GitHub integration for Context Protocol.
...
def create_issue_body(issue: DoctorIssue) -> str:
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Created by `context-protocol doctor --github`*")
...
def create_github_issue(
    ...
    # Add labels
    default_labels = ["context-protocol", issue.severity]
...
def find_existing_issues(
    target_dir: Path,
    label: str = "context-protocol",
) -> List[Dict[str, Any]]:
    """Find existing context-protocol issues in the repo."""
    try:
        result = subprocess.run(
            [
                "gh", "issue", "list",
