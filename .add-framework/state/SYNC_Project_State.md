...
**Key context:**
The CLI is the main deliverable. Understanding `repair.py` is important for working on automated fixes. Each CLI command lives in its own file under `src/add_framework/`.
...
- Named module `add-framework-cli` in modules.yaml
...
|--------|------|------|----------|
| add-framework-cli | `src/add_framework/**` | `docs/cli/` | CANONICAL |
