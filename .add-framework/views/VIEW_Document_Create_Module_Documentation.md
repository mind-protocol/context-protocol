...
1. **Project state:** `.add-framework/state/SYNC_Project_State.md`
   - Understand project context

2. **Module manifest:** `.add-framework/modules.yaml`
...
3. **The code itself:**
   ...
   - Run `add-framework context {file}` to see what's connected
...
This enables `add-framework context` to find the chain.

### Phase 6: Update Module Manifest

Add or update the mapping in `.add-framework/modules.yaml`:
...
This enables:
- `add-framework validate` to check for unmapped code
...
- [ ] CHAIN links are valid (run `add-framework validate`)
