...
IMPL (planned):  engine/membrane/*
IMPL (planned):  engine/moment_graph/queries.py
IMPL (planned):  engine/moment_graph/traversal.py
IMPL (planned):  engine/moment_graph/surface.py
```
...
```
engine/membrane/frame.py      # ModulationFrame dataclass
engine/membrane/provider.py   # Place-scoped cache + retrieval
engine/membrane/functions.py  # Dynamic modulation functions (O(1))
engine/membrane/compute.py    # Aggregate-to-frame computation (no hot path)
```
...
|-------------|------|--------------|
| Compute frame | `engine/moment_graph/queries.py` | view build / scene query |
| Read frame | `engine/moment_graph/traversal.py` | click/wait traversal |
| Read frame | `engine/moment_graph/surface.py` | surfacing + decay |
| Reset scope | `engine/moment_graph/surface.py` | scene change |
...
<!-- @ngram:todo Decide exact aggregate fields for compute step. -->
<!-- @ngram:todo Define bounds for each modifier. -->
<!-- @ngram:todo Add tests for scope isolation and idempotency. -->
