...
The Cybernetic Studio architecture defines high-level observable behaviors across its component repositories (ngram, blood-ledger) and the shared graph service (owned by `ngram`). These behaviors focus on the interactions and outcomes at the system level, rather than internal component logic.

### B1: Artifacts are Canonical in Repos, Meaning in Graph

```
GIVEN:  A file (code, asset, SYNC markdown) exists in either the 'ngram' or 'blood-ledger' repository.
...
WHEN:   The Studio Platform (ngram) ingests this stimulus and derives L2 stimuli.
...
```
GIVEN:  The `ngram` Studio Platform or `blood-ledger` Game Cartridge attempts to interact with an unavailable graph service.
