"""
GitHub integration for ADD Framework.
...
def create_issue_body(issue: DoctorIssue) -> str:
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Created by `add-framework doctor --github`*")
...
def create_github_issue(
    ...
    # Add labels
    default_labels = ["add-framework", issue.severity]
...
def find_existing_issues(
    target_dir: Path,
    label: str = "add-framework",
) -> List[Dict[str, Any]]:
    """Find existing add-framework issues in the repo."""
    try:
        result = subprocess.run(
            [
                "gh", "issue", "list",
