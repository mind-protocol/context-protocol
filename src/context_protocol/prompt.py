"""
Prompt command for Context Protocol CLI.
...
def generate_bootstrap_prompt(target_dir: Path) -> str:
    ...
    5. Executing
    """
    return f'''## Context Protocol Bootstrap

You are working on a project with the Context Protocol installed. Before doing anything, follow these steps:

### Step 1: Understand the Protocol

Read these files to understand how to work:
```
{target_dir}/.context-protocol/PROTOCOL.md    # Navigation — what to load, where to update
{target_dir}/.context-protocol/PRINCIPLES.md  # Stance — how to work well
```
...
```
{target_dir}/.context-protocol/state/SYNC_Project_State.md
```
...
Based on what you need to do, load ONE view from `.context-protocol/views/`:
