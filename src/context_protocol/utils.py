"""
Shared utilities for Context Protocol CLI.
"""
...
def get_templates_path() -> Path:
    ...
    if package_templates.exists() and (package_templates / "context-protocol").exists():
        return package_templates

    # Location 2: Repo root (development mode)
    # utils.py -> context_protocol/ -> src/ -> repo_root (3 levels)
    repo_root = Path(__file__).parent.parent.parent
    repo_templates = repo_root / "templates"
    if repo_templates.exists() and (repo_templates / "context-protocol").exists():
