"""
Prompt command for ADD Framework CLI.
...
def generate_bootstrap_prompt(target_dir: Path) -> str:
    ...
    5. Executing
    """
    return f'''## ADD Framework Bootstrap

You are working on a project with the ADD Framework installed. Before doing anything, follow these steps:

### Step 1: Understand the Protocol

Read these files to understand how to work:
```
{target_dir}/.add-framework/PROTOCOL.md    # Navigation — what to load, where to update
{target_dir}/.add-framework/PRINCIPLES.md  # Stance — how to work well
```
...
```
{target_dir}/.add-framework/state/SYNC_Project_State.md
```
...
Based on what you need to do, load ONE view from `.add-framework/views/`:
